...
class Simulator {
    ...
    public static void main(String[] args) throws Exception
    {
		...
		try {
		    for (int a = 0 ; a != args.length ; ++a)
				if (args[a].equals("-g") || args[a].equals("--groups")) {
				    ...
				    if (++a == args.length)
				    	throw new IllegalArgumentException("Missing stats file");
				    stats_file = args[a];
				} else if (args[a].equals("--repeats")) {
				    if (++a == args.length)
				    	throw new IllegalArgumentException("Missing repeat number");
				    repeats = Integer.parseInt(args[a]);
		...
		if (stats_file != null)
			stats_output = new PrintStream(new FileOutputStream(stats_file, false));
		
		for (int round = 1; round <= repeats; ++ round) {
			AtomicInteger score = new AtomicInteger(0);
			List<Record> stats = new ArrayList<Record>();
		    ...
		    Record last = stats.get(stats.size() - 1);
			...
			if (tournament_path == null) {
			    ...
			    System.err.println("\tNumber of empty cells: " + last.empty);
			    System.err.println("\tNumber of road cells: " + last.road);
		...
		if (stats_output != null) stats_output.close();
		System.err.println("Summary :");
	    System.err.println("\tAverage score: " + ((double)ag_score) / repeats);
    ...
    private static boolean play(String group,
			    ...
			    long cpu_time_ms,
			    AtomicInteger score,
			    List<Record> stats) throws Exception
    {
		int numRejects = 0;
		...
		do {
			...
			stats.add(new Record(land, score.get()));
    ...
    public static void gui(HTTPServer server, String content) throws UnknownServiceException {
		String path = null;
		for (;;) {
		    // get request
		    for (;;)
